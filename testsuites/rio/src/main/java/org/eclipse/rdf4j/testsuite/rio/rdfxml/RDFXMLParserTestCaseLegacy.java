 ...
 */
public abstract class RDFXMLParserTestCaseLegacy {
	...
	private static final String LOCAL_TESTS_DIR = "/testcases/rdfxml/legacy/";
	...
	public TestSuite createTestSuite() throws Exception {
		...
		// Create test suite
		TestSuite suite = new TestSuite(RDFXMLParserTestCaseLegacy.class.getName());
	...
	private static URL resolveURL(String urlString) throws MalformedURLException {
		if (urlString.startsWith(W3C_TESTS_DIR)) {
			...
			urlString = LOCAL_TESTS_DIR + urlString.substring(W3C_TESTS_DIR.length());
		}

		if (urlString.startsWith("/")) {
			return RDFXMLParserTestCaseLegacy.class.getResource(urlString);
		} else {
